...
class GameOverDisplay:
    ...
    def draw(self):
        ...
        ov = pygame.Surface((SCREEN_W, SCREEN_H), pygame.SRCALPHA).convert_alpha()
        ov.fill((0, 0, 0, 185))
        ...
        winner_text = self.game.font_xl.render(text, True, color).convert_alpha()
        scores = f"Player: {self.game.board.player_mana} mana   |   AI: {self.game.board.ai_mana} mana"
        mana_scores_text = self.game.font_md.render(scores, True, PanelColors.TEXT).convert_alpha()
        play_again_text = self.game.font_sm.render("Press  R  to play again", True, PanelColors.TEXT_DIM).convert_alpha()
