...
class Game:
    ...
    def __init__(self):
        ...
        self.freeze_btn_rect = pygame.Rect(start_x + btn_width + spacing, btn_y, btn_width, btn_height)
    ...
    def is_valid_spell_area(self, cur_x, cur_y, gx, gy):
        if self.map[gy][gx] != ' ': return False
    ...
    def handle_player_spell(self, mouse_pos):
        ...
        if self.is_valid_spell_area(self.player_x, self.player_y, gx, gy):
            self.map[gy][gx] = 'burn' if self.selected_spell == SpellOption.BURN else 'freeze'
    ...
    def draw_ui(self):
        ...
        ui_rect = pygame.Rect(0, self.grid_height, self.width, self.UI_PANEL_HEIGHT)
        ...
        if self.current_turn == TurnState.PLAYER_SPELL:
            # Burn Button styling
            burn_color = self.COLOR_BTN_ACTIVE if self.selected_spell == SpellOption.BURN else self.COLOR_BTN_INACTIVE
            pygame.draw.rect(self.screen, burn_color, self.burn_btn_rect)
            burn_text = self.font.render("BURN", True, (255, 255, 255))
            self.screen.blit(burn_text, (self.burn_btn_rect.x + 35, self.burn_btn_rect.y + 7))

            # Freeze Button styling
            freeze_color = self.COLOR_BTN_FREEZE if self.selected_spell == SpellOption.FREEZE else self.COLOR_BTN_INACTIVE
            pygame.draw.rect(self.screen, freeze_color, self.freeze_btn_rect)
            freeze_text = self.font.render("FREEZE", True, (255, 255, 255))
            self.screen.blit(freeze_text, (self.freeze_btn_rect.x + 25, self.freeze_btn_rect.y + 7))
        else:
            # Display informational text when it's not the spell state
            status_text = self.font.render("Move your character to select a spell...", True, (150, 150, 150))
    ...
    def handle_player_movement(self, mouse_pos):
        ...
        if target_tile == ' ' and (distance_x <= 1 and distance_y <= 1) and (distance_x + distance_y > 0):
    ...
    def simulate_bot_move(self):
        while True:
            ...
            if (random_y == 0 and random_x == 0): continue
    ...
    def handle_turn(self, mouse_pos=None):
        if self.current_turn == TurnState.PLAYER_MOVE:
            ...
            if self.selected_spell == None: self.check_ui_clicks(mouse_pos)
            ...
            for gy, row in enumerate(self.map):
                for gx, tile in enumerate(row):
                    if tile == 'x':
                        ...
                        tile_color = self.COLOR_BTN_ACTIVE
                    elif tile == 'freeze':
